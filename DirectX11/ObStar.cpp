...
void ObStar::Render()
{
    ...
    LineTo(g_hdc, position.x + 100 * cosf(144 * ToRadian + rotation) * scale.x,
        position.y + 100 * sinf(144 * ToRadian + rotation) * scale.y);

    LineTo(g_hdc, position.x + 100 * cosf(288 * ToRadian + rotation) * scale.x,
        position.y + 100 * sinf(288 * ToRadian + rotation) * scale.y);

    LineTo(g_hdc, position.x + 100 * cosf(72 * ToRadian + rotation) * scale.x,
        position.y + 100 * sinf(72 * ToRadian + rotation) * scale.y);

    LineTo(g_hdc, position.x + 100 * cosf(216 * ToRadian + rotation) * scale.x,
